...
class areapartos_desk extends StatelessWidget {
...
class _regis1State extends State<regis1> {
  ...
  Future<void> consultahembra() async {
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection('registro_hembra').get();
    List<int> dataList = [];
  ...
  Future<void> consultarNombre(int idSeleccionado) async {
    ...
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance
        ...
        .where('id', isEqualTo: idSeleccionado)
        .get();
  ...
  void actualizarKgDia() {
    // Obtener el valor actual de lechones
    int numeroLechones = int.tryParse(numerolechones.text) ?? 0;

    // Obtener el valor actual de raciones
    int numeroRaciones = int.tryParse(numeroderacionescontroller.text) ?? 1;

    // Calcular el nuevo valor de kg día multiplicando por el número de raciones
    double nuevoValorKgDia = numeroLechones * 0.5 * numeroRaciones;

    // Actualizar el controlador de kg día con el nuevo valor
    kgdiacontroller.text = nuevoValorKgDia.toString();

    // Calcular el valor de kg total multiplicando por 7
    double kgTotal = nuevoValorKgDia * 7;

    // Actualizar el controlador de kg total con el nuevo valor concatenado con "kg"
    kgtotalsemana.text = kgTotal.toString();

    // Imprimir el resultado en la consola
    print('Nuevo valor de Kg Día: $nuevoValorKgDia');
    print('Nuevo valor de Kg Total: $kgTotal');
  }
  ...
  Container numerolechone() {
    ...
    return Container(
      height: 13 * fem,
      child: TextFormField(
        controller: numerolechones,
        keyboardType: TextInputType.number,
        onChanged: (value) {
          // Cuando el usuario ingresa un número de lechones, actualizar el kg día
          actualizarKgDia();
        },
        style: TextStyle(
          fontSize: 3 * fem,
        ...
        decoration: InputDecoration(
          prefixIcon: Icon(Icons.more_time_outlined,
              color: Color.fromRGBO(229, 203, 206, 1), size: 25),
          hintText: 'Ingrese lechones',
          hintStyle: TextStyle(
            fontSize: 3 * fem,
            color: Colors.grey,
          ),
          labelText: 'NUMERO LECHONES',
          labelStyle: TextStyle(
            fontSize: 3 * fem,
            color: Colors.grey,
          ),
